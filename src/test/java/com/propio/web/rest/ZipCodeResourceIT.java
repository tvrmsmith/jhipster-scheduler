...
class ZipCodeResourceIT {
    ...
    void putWithMissingIdPathParamZipCode() throws Exception {
        ...
        restZipCodeMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(zipCode))
            )
    ...
    void deleteZipCode() throws Exception {
        ...
        // Delete the zipCode
        restZipCodeMockMvc
            .perform(delete(ENTITY_API_URL_ID, zipCode.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
